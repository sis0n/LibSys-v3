window.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        if (savedPage) {
            ...
            else sessionStorage.removeItem('bookCatalogPage_faculty');
        }

        const savedCampus = sessionStorage.getItem('bookCatalogCampus_faculty');
        const savedCampusText = sessionStorage.getItem('bookCatalogCampusText_faculty');
    ...
    if (campusBtn && campusMenu) {
        ...
        window.selectCampus = function (el, value, text) {
            ...
            currentPage = 1;
            try {
                sessionStorage.setItem('bookCatalogCampus_faculty', value);
                sessionStorage.setItem('bookCatalogCampusText_faculty', text);
                sessionStorage.removeItem('bookCatalogPage_faculty');
            } catch (e) { }
            loadBooks(1);
            loadAvailableCount();
        }
    ...
    if (statusBtn && statusMenu) {
        ...
        window.selectStatus = function (el, value) {
            ...
            closeAllMenus();
            currentPage = 1;
            try { sessionStorage.removeItem('bookCatalogPage_faculty'); } catch (e) { }
            loadBooks(1);
        }
    }
    ...
    async function loadBooksInitial(page = 1) {
        ...
        try {
            ...
            try { sessionStorage.setItem('bookCatalogPage_faculty', currentPage); } catch(e){}
        } catch (err) {
    ...
    async function loadAvailableCount() {
        try {
            ...
            if (campusValueFilter !== "all") params.set('campus_id', campusValueFilter);
            const r = await fetch(`api/faculty/bookCatalog/availableCount?${params.toString()}`);
            if (!r.ok) throw Error();
            const d = await r.json();
