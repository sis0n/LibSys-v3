window.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        const savedCampus = sessionStorage.getItem('bookCatalogCampus');
        const savedCampusText = sessionStorage.getItem('bookCatalogCampusText');
        if (savedCampus && savedCampusText) {
            campusValueFilter = savedCampus;
            if (campusValue) campusValue.textContent = savedCampusText;
        } else {
            // Default to user's home campus from hidden inputs
    ...
    if (campusBtn && campusMenu) {
        ...
        window.selectCampus = function (el, value, text) {
            ...
            currentPage = 1;
            try {
                sessionStorage.setItem('bookCatalogCampus', value);
                sessionStorage.setItem('bookCatalogCampusText', text);
        ...
        // Highlight selected
        const items = campusMenu.querySelectorAll(".campus-item");
        items.forEach(item => {
            if (item.textContent.trim() === (campusValue ? campusValue.textContent.trim() : "")) {
                item.classList.add("bg-[var(--color-orange-200)]", "font-semibold");
            }
        });
    }
    ...
    async function loadBooksInitial(page = 1) {
        ...
        try {
            ...
            try { sessionStorage.setItem('bookCatalogPage', currentPage); } catch(e){}
        } catch (err) {
    ...
    async function loadAvailableCount() {
        try {
            ...
            if (campusValueFilter !== "all") params.set('campus_id', campusValueFilter);
            const r = await fetch(`api/student/bookCatalog/availableCount?${params.toString()}`);
            if (!r.ok) throw Error();
            const d = await r.json();
