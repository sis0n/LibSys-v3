window.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCampuses(currentCampusId = null) {
      ...
      try {
          const res = await fetch(`${window.BASE_URL}/api/campuses/all`);
  ...
  async function loadProfile() {
    try {
      const res = await fetch(`${window.BASE_URL}/api/librarian/myProfile/get`); 
      const data = await res.json();
  ...
  profileForm.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch(`${window.BASE_URL}/api/librarian/myProfile/update`, {
        method: 'POST',
        headers: {
