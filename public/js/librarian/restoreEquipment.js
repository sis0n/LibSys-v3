document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadDeleted() {
        ...
        try {
            const res = await fetch(`api/librarian/restoreEquipment/fetch`);
    ...
    window.restoreEq = async (id, name) => {
        ...
        if (confirm.isConfirmed) {
            const res = await fetch(`api/librarian/restoreEquipment/restore/${id}`, { method: "POST" });
            const data = await res.json();
            if (data.success) {
    ...
    window.permanentDeleteEq = async (id, name) => {
        ...
        if (confirm.isConfirmed) {
            const res = await fetch(`api/librarian/restoreEquipment/deletePermanently/${id}`, { method: "POST" });
            const data = await res.json();
            if (data.success) {
