...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function populateCirculatedBooks(filter = 'month') {
        ...
        try {
            const response = await fetch(`${BASE_URL}/api/librarian/reports/circulated-books?filter=${filter}`);
    ...
    async function populateCirculatedEquipments(filter = 'month') {
        ...
        tbody.innerHTML = '<tr><td colspan="5" class="text-center p-4"><i class="ph ph-spinner animate-spin text-lg mr-2"></i>Loading...</td></tr>';
        try {
            const response = await fetch(`${BASE_URL}/api/librarian/reports/circulated-equipments?filter=${filter}`);
            const result = await response.json();
            tbody.innerHTML = '';
            if (result.data && result.data.length > 0) {
                result.data.forEach(row => {
    ...
    async function populateDeletedBooks(filter = 'month') {
        ...
        try {
            const response = await fetch(`${BASE_URL}/api/librarian/reports/deleted-books?filter=${filter}`);
            const result = await response.json();
            tbody.innerHTML = '';
    ...
    async function populateLibraryVisitByDepartment(filter = 'month') {
        ...
        tbody.innerHTML = '<tr><td colspan="5" class="text-center p-4"><i class="ph ph-spinner animate-spin text-lg mr-2"></i>Loading...</td></tr>';
        try {
            const response = await fetch(`${BASE_URL}/api/librarian/reports/library-visits-department?filter=${filter}`);
            const result = await response.json();
            tbody.innerHTML = '';
            if (result.success && result.data && result.data.length > 0) {
                result.data.forEach(row => {
    ...
    async function populateMostBorrowedBooks(filter = 'month') {
        ...
        tbody.innerHTML = '<tr><td colspan="4" class="text-center p-4"><i class="ph ph-spinner animate-spin text-lg mr-2"></i>Loading...</td></tr>';
        try {
            const response = await fetch(`${BASE_URL}/api/librarian/reports/most-borrowed-books?filter=${filter}`);
            const result = await response.json();
            tbody.innerHTML = '';
            if (result.success && result.data && result.data.length > 0) {
    ...
    async function initializeCharts(filter = 'month') {
        ...
        try {
            const res = await fetch(`${BASE_URL}/api/librarian/reports/getActivityReport?filter=${filter}`);
