document.addEventListener("DOMContentLoaded", () => {
  ...
  const fetchBooks = async () => {
    try {
      ...
      const response = await fetch(
        `api/superadmin/booksmanagement/fetch?${params}`,
      );
  ...
  window.viewBook = async (id) => {
    try {
      const response = await fetch(
        `api/superadmin/booksmanagement/details/${id}`,
      );
      const data = await response.json();
      if (data.success) {
  ...
  window.editBook = async (id) => {
    try {
      ...
      if (data.success) {
        ...
        document.getElementById("edit_borrowing_duration_override").value = book.borrowing_duration_override || "";
  ...
  editBookForm.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(
        `api/superadmin/booksmanagement/update/${bookId}`,
        {
          method: "POST",
          body: formData,
        },
