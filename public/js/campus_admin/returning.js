document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchRecentReturns() {
    try {
      const response = await fetch(
        "api/campus_admin/returning/getRecent?limit=10",
      );
  ...
  async function handleBookCheck(accessionNumber) {
    ...
    try {
      const response = await fetch("api/campus_admin/returning/checkBook", {
        method: "POST",
        body: formData,
      });
      const result = await response.json();
  ...
  if (modalReturnButton) {
    modalReturnButton.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch("api/campus_admin/returning/markReturned", {
          method: "POST",
          body: formData,
        });
        const result = await response.json();
  ...
  if (modalExtendButton) {
    modalExtendButton.addEventListener("click", async () => {
      ...
      if (days) {
        ...
        const response = await fetch("api/campus_admin/returning/extend", {
          method: "POST",
          body: formData,
        });
        const result = await response.json();
