...
class StaffTicketRepository
{
  ...
  public function getStaffFullInfoByUserId(int $userId): ?array
  {
    $stmt = $this->db->prepare("
        SELECT 
            s.staff_id, s.employee_id, s.position,
  ...
  {
    $stmt = $this->db->prepare("SELECT staff_id FROM staff WHERE user_id = ?");
    $stmt->execute([$userId]);
  ...
  public function getStaffCartItems(int $userId): array
  {
    $stmt = $this->db->prepare("
  ...
  public function getStaffCartItemsByIds(int $userId, array $cartIds): array
  {
  ...
  public function removeStaffCartItemsByIds(int $userId, array $cartIds): void
  {
    if (empty($cartIds)) return;
    $placeholders = implode(',', array_fill(0, count($cartIds), '?'));
  ...
  // --- TRANSACTION LOGIC ---
  public function createPendingTransactionForStaff(int $staffId, string $transactionCode, string $dueDate, string $qrPath, int $expiryMinutes = 15): int
  {
    $stmt = $this->db->prepare("
            INSERT INTO borrow_transactions 
            (staff_id, transaction_code, qrcode, due_date, status, generated_at, expires_at)
  ...
  {
    $stmt = $this->db->prepare("SELECT * FROM borrow_transactions WHERE staff_id = :sid AND status = 'pending' LIMIT 1");
    $stmt->execute(['sid' => $staffId]);
    return $stmt->fetch(PDO::FETCH_ASSOC) ?: null;
  }
  ...
  {
    $stmt = $this->db->prepare("SELECT transaction_id FROM borrow_transactions WHERE status = 'pending' AND expires_at <= NOW() AND staff_id IS NOT NULL");
    $stmt->execute();
